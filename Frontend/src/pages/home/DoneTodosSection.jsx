...
const DoneTodosSection = () => {
  ...
  const fetchTodos = async () => {
    const status = "done"; // filter done todos
    try {
  ...
  return (
    <>
      ...
      {todos.length > 0 ? (
        <div
          id="done-section"
          className="w-[80vw] max-w-5xl bg-ywSoft m-auto mt-2 rounded-2xl flex flex-wrap overflow-hidden gap-6 justify-between px-10 py-6"
        >
          {todos.map((todo) => (
            <TodoCart
              todo={todo}
              ...
              editbtn={false} // Done todos में edit button hide
            />
          ))}
        </div>
      ) : (
      ...
      {todos.length > 0 && (
        <div className="flex items-center justify-between w-[80vw] max-w-5xl mx-auto mt-4 px-8 py-2 bg-white shadow-md rounded-xl gap-3 flex-wrap">
          ...
          <button
            ...
            disabled={page === 1}
            className={`px-3 sm:px-4 py-2 rounded-lg text-white font-medium transition text-sm sm:text-base ${
              page === 1
...
export default DoneTodosSection;
